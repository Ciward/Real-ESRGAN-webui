...
with gr.Blocks(title="Real-ESRGAN") as demo:
  ...
  with gr.Tabs() as tab:
    with gr.TabItem("Restore Image"):
      with gr.Row():
        with gr.Column():
          image_input = gr.Image(label="Input", image_mode="RGBA", height=300)
          # 添加示例图片
          img_list = [img for img in os.listdir(osp.join(REAL_ESRGAN_DIR, "inputs")) if img.endswith(('.png', '.jpg', '.jpeg'))]
          gr.Examples(inputs=image_input,examples=list(map(lambda input_file:osp.join(REAL_ESRGAN_DIR, "inputs", input_file),img_list)))
        with gr.Column():
          image_output = gr.Image(label="Output", interactive=False, image_mode="RGBA", height=300)
  ...
  with gr.Row():
    with gr.Column():
      ...
      with gr.Row():
        face_enhance = gr.Checkbox(label="Face enhance", info="Use GFPGAN to enhance face",visible=False)
        fp32 = gr.Checkbox(label="Use fp32", info="Default: fp16 (half precision).", interactive=True,visible=False)
        alpha_upsampler = gr.Radio(choices=["realesrgan", "bicubic"], value="realesrgan", label="Alpha upsampler", interactive=True,visible=False)
